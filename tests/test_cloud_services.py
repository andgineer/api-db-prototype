...
def test_send_email():
    with patch("cloud_services.ses") as mock_ses:
        ...
        assert (
            mock_ses.return_value.send_email.call_args[1]["Message"]["Body"]["Html"]["Data"] == html
        )
        assert (
            mock_ses.return_value.send_email.call_args[1]["Message"]["Body"]["Text"]["Data"] == text
        )


def test_get_queue_message():
    with (
        patch("cloud_services.sqs") as mock_sqs,
        patch("cloud_services.queue_url") as mock_queue_url,
    ):
        mock_sqs.return_value.receive_message.return_value = {
            "Messages": [{"MessageId": "123456789", "ReceiptHandle": "handle"}]
        }
