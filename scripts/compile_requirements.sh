...
uv pip compile requirements.prod.in --output-file=requirements.prod.txt --upgrade
END_TIME=$(date +%s)

echo "Req‘s compilation time: $((REQS_TIME - $START_TIME)) seconds"
echo "Req‘s dev compilation time: $((REQS_DEV_TIME - REQS_TIME)) seconds"
echo "Req‘s prod compilation time: $((END_TIME - $REQS_DEV_TIME)) seconds"
